cat > /mnt/user-data/outputs/forgot.blade.php << 'HTMLEOF'
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* INPUT WA — muncul/hilang */
    .wa-field { overflow: hidden; max-height: 0; opacity: 0; transition: max-height 0.3s ease, opacity 0.3s ease, margin 0.3s ease; margin-bottom: 0; }
    .wa-field.show { max-height: 80px; opacity: 1; margin-bottom: 16px; }
    ...
    <div class="auth-right">
        ...
        <div class="auth-card">
            ...
            <form method="POST" action="{{ route('password.sendOtp') }}">
                ...
                {{-- Input No HP — muncul hanya saat pilih WhatsApp --}}
                <div class="wa-field" id="waField">
                    <div class="input-wrap">
...
<script>
    ...
    function toggleWaField() {
        if (optWa.checked) {
            waField.classList.add('show');
            phoneInput.required = true;
        } else {
            waField.classList.remove('show');
            phoneInput.required = false;
            phoneInput.value = '';
        }
    }

    optEmail.addEventListener('change', toggleWaField);
    optWa.addEventListener('change', toggleWaField);

    // Cek saat load (kalau old value WA)
    toggleWaField();
</script>
