...
{
    // Normalisasi blok dulu
    $blok = strtoupper(trim($request->blok));

    // ✅ Cek duplikat kombinasi blok + no_rumah
    $sudahAda = \App\Models\Rumah::whereRaw('UPPER(TRIM(blok)) = ?', [$blok])
        ->where('no_rumah', $request->no_rumah)
        ->exists();
    ...
    \App\Models\Rumah::create([
        'blok'      => $blok,  // simpan dalam format uppercase
        'no_rumah'  => $request->no_rumah,
        'status'    => $request->status,
        'luas_tanah'=> $request->luas_tanah,
        'harga'     => $request->harga,
