...
class PenghuniController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //   // Cek duplikat blok + no_rumah
        //     $sudahAda = \App\Models\Rumah::where('blok', $request->blok)
        //         ->where('no_rumah', $request->no_rumah)
        //         ->exists();

        //    if ($sudahAda) {
        //         return redirect()->back()
        //             ->with('error', 'Kavling ' . $request->blok . ' No. ' . $request->no_rumah . ' sudah ada!');
        //     }
...
{
    // Normalisasi blok — hapus prefix "blok" lalu uppercase
    $blok = strtoupper(trim($request->blok));
    $blok = trim(preg_replace('/^BLOK\s+/i', '', $blok)); // "Blok A" → "A"

    // Ambil semua rumah lalu normalisasi di PHP untuk cek duplikat
    $sudahAda = \App\Models\Rumah::where('no_rumah', $request->no_rumah)
        ->get()
        ->contains(function ($r) use ($blok) {
            $dbBlok = strtoupper(trim($r->blok));
            $dbBlok = trim(preg_replace('/^BLOK\s+/i', '', $dbBlok));
            return $dbBlok === $blok;
        });

    if ($sudahAda) {
        return redirect()->back()
            ->with('error', 'Kavling Blok ' . $blok . ' No. ' . $request->no_rumah . ' sudah terdaftar!');
    }
    ...
    $rtId = $this->getRtOtomatis($blok);

    if (!$rtId) {
        return redirect()->back()->with('error', 'Blok tidak dikenali!');
    }

    \App\Models\Rumah::create([
        'blok'       => $blok,  // disimpan konsisten: "A", "B", dst
        'no_rumah'   => $request->no_rumah,
        'status'     => $request->status,
        'luas_tanah' => $request->luas_tanah,
        'harga'      => $request->harga,
