...
{
    // Jangan langsung lunas, redirect ke halaman upload bukti
    return redirect()->route('user.iuran.upload', $id)
        ->with('info', 'Setelah bayar via QRIS, upload bukti pembayaran di bawah ini');
}
        ...
        public function storeUpload(Request $request, $id)
    {
        ...
        if ($request->hasFile('bukti_pembayaran')) {
            ...
            $iuran->update([
                ...
                'status' => 'menunggu', // ✅ GANTI dari 'belum' ke 'menunggu'
            ]);
        }

        return redirect()->route('user.iuran.index')
            ->with('success', 'Bukti berhasil diupload, menunggu konfirmasi RT');
    }
