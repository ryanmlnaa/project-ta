  ...
  public function storeRumah(Request $request)
{
    ...
    $blok = trim(preg_replace('/^BLOK\s+/i', '', $blok));

    $noRumah = $request->no_rumah;

    // Ambil semua rumah, normalisasi di PHP, cek manual
    $semuaRumah = \App\Models\Rumah::all();

    $sudahAda = false;
    foreach ($semuaRumah as $r) {
        $dbBlok = strtoupper(trim($r->blok));
        $dbBlok = trim(preg_replace('/^BLOK\s+/i', '', $dbBlok));

        if ($dbBlok === $blok && (string)$r->no_rumah === (string)$noRumah) {
            $sudahAda = true;
            break;
        }
    }
    ...
    if ($sudahAda) {
        return redirect()->back()
            ->with('error', 'Kavling Blok ' . $blok . ' No. ' . $noRumah . ' sudah terdaftar!');
    }
