...
}eturn back()->with('error', 'Kavling ' . $request->blok . ' No. ' . $request->no_rumah . ' sudah ada!');
            }
    ...
    public function storeRumah(Request $request)
    {
        $request->validate([
            'blok' => 'required',
             'no_rumah' => [
            'required',
                \Illuminate\Validation\Rule::unique('rumah')->where(function ($query) use ($request) {
                    return $query->where('blok', $request->blok);
                }),
            ],
            'status' => 'required',
        ]);
