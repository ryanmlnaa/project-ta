...
class UserIuranController extends Controller
{
    ...
    public function storeUpload(Request $request, $id)
    {
        ...
        if ($request->hasFile('bukti_pembayaran')) {
            ...
            $iuran->update([
                ...
                'status' => 'belum' // menunggu approve admin
            ]);
        }

        return redirect()->route('user.iuran.index')
            ->with('success', 'Bukti berhasil diupload, menunggu konfirmasi admin');
    }
   ...
   public function status()
    ...
    {
        $iuran = \App\Models\Iuran::findOrFail($id);

        // opsional: hanya milik user
        iif ($iuran->penghuni_id != Auth::user()->id) {
            abort(403);
        }
