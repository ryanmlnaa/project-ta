...
class IuranController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($penghuni && $penghuni->telepon) {
            ...
            $pesan = "🏠 *GREEN VIEW - Tagihan Iuran Baru*\n\n";
            $pesan .= "Halo *{$penghuni->nama}*,\n";
            $pesan .= "Anda memiliki tagihan iuran baru:\n\n";
            ...
            $pesan .= "Silakan bayar via QRIS yang tersedia di aplikasi.\n";
            $pesan .= "Setelah bayar, upload bukti pembayaran di halaman Iuran.\n\n";
    ...
    public function approveRT($id)
    {
        ...
        if ($penghuni && $penghuni->telepon) {
            $nohp = preg_replace('/^0/', '62', $penghuni->telepon);
            $pesan = "✅ *GREEN VIEW - Pembayaran Dikonfirmasi*\n\n";
            $pesan .= "Halo *{$penghuni->nama}*,\n";
            $pesan .= "Pembayaran iuran Anda telah *diverifikasi* oleh RT.\n\n";
            $pesan .= "📅 Bulan   : {$iuran->bulan} {$iuran->tahun}\n";
            $pesan .= "💰 Jumlah  : Rp " . number_format($iuran->jumlah,0,',','.') . "\n";
            $pesan .= "📋 Jenis   : {$iuran->jenis_iuran}\n";
            $pesan .= "📝 Keterangan: " . ($iuran->keterangan ?? '-') . "\n";
            $pesan .= "📆 Tgl Lunas: " . now()->format('d M Y') . "\n\n";
            $pesan .= "Status pembayaran: *LUNAS* ✅\n\n";
            $pesan .= "_Terima kasih telah membayar tepat waktu_ 🙏\n";
            $pesan .= "— RT Green View";
