...
class UserController extends Controller
{
    ...
    // }
        ...
        return view('user.home', compact('penghuni', 'iuran'));
    }
    // =========================
    // PROFIL
    // =========================
  ...
  public function simpanRumah(Request $request)
{
    ...
    // 🔥 Ambil penghuni berdasarkan user login
    $penghuni = \App\Models\Penghuni::where('user_id', Auth::id())->first();

    if (!$penghuni) {
        return back()->with('error', 'Isi data penghuni dulu!');
    }

    // 🔥 UPDATE penghuni -> isi rumah_id
    $penghuni->rumah_id = $request->rumah_id;
    $penghuni->save();

    // 🔥 UPDATE status rumah jadi TERISI
    \App\Models\Rumah::where('id', $request->rumah_id)
