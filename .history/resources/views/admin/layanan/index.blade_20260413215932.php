                                        ...
                                        <form id="formTanggapi" method="POST">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <textarea name="tanggapan_admin" class="form-control mb-2"
                                                        required></textarea>
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            let table = $('#dataTable').DataTable({
                ...
                lengthChange: false,
                ordering: true,
                destroy: true, // 🔥 penting biar tidak double init
                language: {
                    search: "Cari:",
                    paginate: {
                        previous: "Sebelumnya",
            ...
            // 🔥 FIX EVENT (PAKAI OFF + ON)
            $(document).off('click', '.btn-tanggapi').on('click', '.btn-tanggapi', function () {

                let id = $(this).data('id');

                let action = "{{ url('/layanan') }}/" + id + "/tanggapi";

                $('#formTanggapi').attr('action', action);

                // 🔥 RESET FORM (BIAR TIDAK BUG)
                $('#formTanggapi')[0].reset();
            });
    ...
    <script>
        $('#modalTanggapi').on('show.bs.modal', function () {
            $('body').addClass('modal-open');
        });

        $('#modalTanggapi').on('hidden.bs.modal', function () {
