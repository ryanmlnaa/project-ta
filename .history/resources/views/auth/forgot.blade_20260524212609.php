...
<head>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .method-group { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 16px; }
    .method-option { display: none; }
    .method-card { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 5px; padding: 12px 8px; border-radius: 12px; border: 2px solid rgba(255,255,255,0.2); background: rgba(255,255,255,0.08); cursor: pointer; transition: all 0.2s; user-select: none; }
    ...
    .method-option:checked + .method-card { border-color: #ffffff; background: rgba(255,255,255,0.2); box-shadow: 0 0 0 3px rgba(255,255,255,0.15); }
    ...
    /* ✅ BOX INPUT NOMOR WA */
    .wa-phone-box { display: none; margin-bottom: 16px; }
    .wa-phone-box.show { display: block; }
    .wa-phone-label { color: rgba(255,255,255,0.8); font-size: 12px; font-weight: 600; margin-bottom: 6px; display: block; }
    .wa-phone-hint { font-size: 11px; color: rgba(255,255,255,0.55); margin-top: 5px; }

    .wa-info { background: rgba(255,255,255,0.12); border: 1px solid rgba(255,255,255,0.25); border-radius: 10px; padding: 10px 14px; font-size: 12px; color: rgba(255,255,255,0.85); line-height: 1.5; margin-bottom: 10px; }
        ...
        <div class="auth-card">
            ...
            <form method="POST" action="{{ route('password.sendOtp') }}">
                ...
                <div class="method-group">
                    <label>
                        <input type="radio" name="otp_via" value="email" class="method-option" id="opt_email" checked>
                    ...
                    <label>
                        <input type="radio" name="otp_via" value="whatsapp" class="method-option" id="opt_wa">
                        <div class="method-card">
                            <span class="mc-icon">💬</span>
                            <span class="mc-title">WhatsApp</span>
                            <span class="mc-sub">masukkan nomor HP</span>
                        </div>
                    </label>
                </div>

                {{-- ✅ INPUT NOMOR WA — muncul saat pilih WhatsApp --}}
                <div class="wa-phone-box" id="waPhoneBox">
                    <div class="wa-info">
                        💡 Masukkan nomor WhatsApp aktif Anda untuk menerima kode OTP.
                    </div>
                    <span class="wa-phone-label">Nomor WhatsApp</span>
                    <div class="input-wrap">
                    ...
                    <span class="wa-phone-hint">Format: 08xxxxxxxxxx atau +628xxxxxxxxx</span>
                </div>
...
<script>
    const optEmail   = document.getElementById('opt_email');
    const optWa      = document.getElementById('opt_wa');
    const waPhoneBox = document.getElementById('waPhoneBox');
    const phoneInput = document.getElementById('phoneInput');

    function toggleWaBox() {
        if (optWa.checked) {
            waPhoneBox.classList.add('show');
            phoneInput.setAttribute('required', 'required');
        } else {
            waPhoneBox.classList.remove('show');
            phoneInput.removeAttribute('required');
        }
    }

    optEmail.addEventListener('change', toggleWaBox);
    optWa.addEventListener('change', toggleWaBox);

    // Restore state saat ada old input (misal validasi gagal)
    toggleWaBox();
</script>
</body>
</html>
