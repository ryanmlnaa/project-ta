            ...
            <tr>
                ...
                <td>
                    @if($i->bukti_pembayaran)
                        <a href="{{ asset('bukti/' . $i->bukti_pembayaran) }}" target="_blank">Lihat</a>
                    @else
                        -
                    @endif
                </td>
                <td>
                    ...
                    @elseif($i->status === 'menunggu')
                        <form action="{{ route('bendahara.iuran.konfirmasi', $i->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PATCH')
                            <button class="btn btn-sm btn-success" onclick="return confirm('Konfirmasi iuran ini lunas?')">Konfirmasi Lunas</button>
                        </form>
                    @else
