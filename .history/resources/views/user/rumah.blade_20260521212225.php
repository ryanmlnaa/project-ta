...
@if($penghuni)
    <div class="profile-info-row">
        <div class="profile-info-label">Status Kontrak</div>
        @if($penghuni->status_huni == 'Tetap')
            <span class="pil pil-green">Tetap</span>
        @else
            <span class="pil pil-amber">Kontrak</span>
        @endif
    </div>
@endif
                    ...
                    @if(!$penghuni)
                        <div class="glass-card fade-in" style="animation-delay:0.2s" id="formPenghuniCard">
                            ...
                            <form action="{{ route('user.simpan.penghuni') }}" method="POST">
                                ...
                                <div class="form-grid">
                                    <input type="text" class="gv-input" name="nama"
                                        value="{{ auth()->user()->name }}" readonly>

                                    <input type="text" class="gv-input" name="no_ktp"
                                        placeholder="No KTP" required
                                        value="{{ old('no_ktp') }}">

                                    <input type="text" class="gv-input" name="telepon"
                                        placeholder="No HP" required
                                        value="{{ old('telepon') }}">

                                    <input type="text" class="gv-input" name="alamat"
                                        placeholder="Alamat" required
                                        value="{{ old('alamat') }}">

                                    <select class="gv-input" name="status_huni" id="status_huni"
                                        required onchange="toggleTanggal()">
                                        <option value="">-- Pilih Status --</option>
                                        <option value="Tetap" {{ old('status_huni') == 'Tetap' ? 'selected' : '' }}>Tetap</option>
                                        <option value="Kontrak" {{ old('status_huni') == 'Kontrak' ? 'selected' : '' }}>Kontrak</option>
                                    </select>
                        ...
                        <div class="glass-card fade-in" style="animation-delay:0.25s">
                            ...
                            <div class="rumah-showcase">
                                <div class="rumah-img-frame">
                                    <img src="{{ asset('foto_rumah/' . $penghuni->rumah->foto) }}" class="rumah-photo"
                                        alt="Foto Rumah">
                                    <div class="rumah-img-overlay">
                                        <span class="rumah-blok-badge">Blok {{ $penghuni->rumah->blok }}</span>
                                    </div>
                                </div>
                            ...
                            <div class="kavling-grid">
                                @foreach($rumahList ?? [] as $index => $r)
                                    <button type="button"
                                        ...
                                        style="animation-delay: {{ $index * 0.04 }}s" data-id="{{ $r->id }}"
                                        data-status="{{ $r->status }}" data-blok="{{ $r->blok }}" data-nomor="{{ $r->no_rumah }}"
                                        data-luas="{{ $r->luas_tanah }}" data-harga="{{ $r->harga }}"
...
<script>
    ...
    document.querySelectorAll('.kav-btn').forEach(btn => {
        btn.addEventListener('click', function (e) {
            ...
            // 🔥 Set action form ke route pilih-rumah/{id}
            ...
            document.getElementById('blokBadge').innerText   = 'Blok ' + this.dataset.blok;
