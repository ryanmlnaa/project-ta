...
<div class="prf-wrapper">
    ...
    <div class="prf-grid">
        ...
        <div class="prf-left">
            ...
            <div class="prf-photo-card">
                <div class="prf-avatar-ring">
                    <img src="{{ auth()->user()->photo
                        ? asset('profile/' . auth()->user()->photo) . '?v=' . time()
                        : 'https://i.pravatar.cc/150?img=3' }}"
                        class="prf-avatar" id="avatarPreview" alt="Foto Profil">
                    <div class="prf-avatar-overlay">
                        <i class="fas fa-camera"></i>
                    </div>
                </div>

                <h5 class="prf-name">{{ auth()->user()->name }}</h5>
                <span class="prf-email">{{ auth()->user()->email }}</span>
                ...
                <form action="{{ route('user.upload.photo') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <label class="prf-file-label" for="photoInput">
                        <i class="fas fa-image me-2"></i>
                        <span id="fileLabel">Pilih Foto</span>
                    </label>
                    <input type="file" name="photo" id="photoInput" class="prf-file-input" required
                        onchange="previewPhoto(this)">

                    <button type="submit" class="prf-btn-upload">
                        <i class="fas fa-cloud-upload-alt me-2"></i>Upload Foto
                    </button>
                </form>

                <p class="prf-file-hint">JPG, PNG maks. 2MB</p>
            </div>

            {{-- Info Card --}}
            <div class="prf-info-card">
                <div class="prf-info-row">
                    <span class="prf-info-icon prf-icon-green"><i class="fas fa-user"></i></span>
                    <div>
                        <span class="prf-info-label">Username</span>
                        <span class="prf-info-value">{{ auth()->user()->username ?? '-' }}</span>
                    </div>
                </div>
                <div class="prf-info-row">
                    <span class="prf-info-icon prf-icon-blue"><i class="fas fa-envelope"></i></span>
                    <div>
                        <span class="prf-info-label">Email</span>
                        <span class="prf-info-value">{{ auth()->user()->email }}</span>
                    </div>
                </div>
            </div>
